...
function Index() {
  ...
  const getDates = (startDate, stopDate) => {
    ...
    return dateArray.map((el, index) => {
      return (
        <li key={index}>{new Date(el).toDateString()}</li>
      )
    });
  }
  ...
  return (
    <main id="main">
            ...
            <div className="col-12 col-md-6">
              <div className="right">
                ...
                >
                  ...
                  <ul>
                    <li>
                      <h5>TRIP ID : <span className="pink fw-bold">{QuotationObj._id}</span></h5>
                    </li>
                    <li>As quoted on</li>
                    <li><h5 className="mb-0">{new Date(QuotationObj?.createdAt).toDateString()}  {new Date(QuotationObj?.createdAt).getHours()}:{new Date(QuotationObj?.createdAt).getMinutes()}</h5></li>
                  </ul>
                </div>
                <div className="right-middle">
                  <div className="destination">
                    <h2 className="text-white">{QuotationObj?.leadObj?.clientObj?.name}</h2>
                    <h2 className="text-white">{QuotationObj?.destinationName}</h2>
                  </div>
                  ...
                  <ul className="detail list-circle">
                    <li>{QuotationObj?.durationOfTour} N {parseInt(QuotationObj?.durationOfTour) - 1} D</li>
                    <li>{QuotationObj?.numberOfGuest} Passengers</li>
                    <li>{QuotationObj?.tourListArr && QuotationObj?.tourListArr.length > 0 && new Date(QuotationObj?.tourListArr[0]?.startDate).toDateString()}</li>
                  </ul>
                  <p className="desp">
          ...
          <table className="table table-borderless align-middle">
            ...
            <tbody>
              <tr>
                ...
                <td>{parseInt(QuotationObj?.perPersonAirPortPrice) * parseInt(QuotationObj?.numberOfGuest)}</td>
              </tr>
              <tr>
                <td>LAND</td>
                <td>{QuotationObj?.perPersonLandPrice}</td>
                <td>{parseInt(QuotationObj?.perPersonLandPrice) * parseInt(QuotationObj?.numberOfGuest)}</td>
              </tr>
            </tbody>
            <tfoot>
              <tr>
      ...
      <div className="desp purple-bg py-2 px-4 my-5">
        ...
        <p className="text-white m-0 text-center">As quoted on {new Date(QuotationObj?.createdAt).toDateString()}  {new Date(QuotationObj?.createdAt).getHours()}:{new Date(QuotationObj?.createdAt).getMinutes()}</p>
      </div>
            ...
            <div className="col-12">
              <div className="box">
                ...
                <ul className="list-circle">
                  {
                    QuotationObj?.tourListArr && QuotationObj?.tourListArr.length > 0 && QuotationObj?.tourListArr.map((el, index) => {
                      return (
                        <li key={index}>
        ...
        <div className="container">
          ...
          <div className="position-relative">
            <div className="row">
              ...
              {
                QuotationObj?.costingObj && QuotationObj?.costingObj?.flightDetails && QuotationObj?.costingObj?.flightDetails.length > 0 && QuotationObj?.costingObj?.flightDetails.filter((el, index, arr) => (arr.length - 1) != index).map((el, index, arr) => {
                  return (
                    <div className="col-12" key={index}>
                      <div className={(index % 2) == 0 ? "box" : "box reverse"}>
                        <h6>
              ...
              {
                QuotationObj?.costingObj && QuotationObj?.costingObj?.flightDetails && QuotationObj?.costingObj?.flightDetails.length > 0 && QuotationObj?.costingObj?.flightDetails[QuotationObj?.costingObj?.flightDetails.length - 1] &&
                <div className="destination">
                  <h6>
                    {QuotationObj?.costingObj?.flightDetails[QuotationObj?.costingObj?.flightDetails.length - 1]?.flightName}
                    <img src={images.location} alt="" />
                  </h6>
                </div>
              }
      ...
      <section className="hotels">
        <div className="container">
          ...
          {QuotationObj?.hotelDetail && QuotationObj?.hotelDetail?.length > 0 && QuotationObj?.hotelDetail.map((el, index) => {
            return (
      ...
      <section className="itinerary mb-80">
        <div className="container">
          ...
          <div className="row">
            {
              QuotationObj?.itineraryDetails && QuotationObj?.itineraryDetails.length > 0 && QuotationObj?.itineraryDetails.map((el, index) => {
                return (
                  <div className="col-12">
                    <div className="day">
                      <h4>
          ...
          <div className="row">
            <div className="col-12">
              <div className="inclusions">
                <div className="box">
                  <h4 className="purple bg-white">Hi {QuotationObj?.leadObj?.clientObj?.name} (TRIP ID {QuotationObj?._id})</h4>
                  <ul className="list-circle">
                    <li>
                    ...
                    <li>
                      <b className="pink">{`${QuotationObj?.agentObj?.firstName} ${QuotationObj?.agentObj?.lastName}`} - {QuotationObj?.agentObj?.phone}</b> will be your
            ...
            <div className="col-12">
              <div className="flight-table">
                <table className="table table-bordered">
                  <thead className="purple-bg text-white">
                    <tr>
                      ...
                      {QuotationObj?.paymentObj?.paymentReceviedArr && QuotationObj?.paymentObj?.paymentReceviedArr.length > 0 && QuotationObj?.paymentObj?.paymentReceviedArr.map((el, index) => {
                        return (
                          <th scope="col" className="fw-normal">{index + 1}<sup>st</sup> Installment</th>
                        )
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      ...
                      {QuotationObj?.paymentObj?.paymentReceviedArr && QuotationObj?.paymentObj?.paymentReceviedArr.length > 0 && QuotationObj?.paymentObj?.paymentReceviedArr.map((el, index) => {
                        return (
                          <td key={index} scope="col" className="fw-normal">₹ {el?.installmentAmount} on {new Date(el.receviedDate).toDateString()}</td>
                        )
                      })}
                    </tr>
                  </tbody>
                ...
                <ul className="amount mb-0">
                  <li>
                    <h4>Total cost including taxes and above</h4>
                    <h4 className="pink fw-semibold m-0">₹ {QuotationObj?.paymentObj?.paymentReceviedArr.reduce((acc, el) => acc + parseInt(el.installmentAmount), 0)}</h4>
                  </li>
