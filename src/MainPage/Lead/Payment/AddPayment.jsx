...
export const AddPayment = () => {
    ...
    const payMentInvoiceArr = useSelector((state) => state.paymentInvoice.paymentInvoices);
    const quotationPaymentObj = useSelector((state) => state.payment.paymentObj);
    const perfomaInvoiceObj = useSelector((state) => state.paymentInvoice.paymentInvoiceObj);
    ...
    const handlePaymentInput = (e, index) => {
        ...
        if (name == "installmentAmount") {
            if (value > total || value <= 0) {
                toastError("Ammount must be greate than zero ot less than total Amount");
            }
        }
    ...
    const handleSubmit = () => {
        ...
        const validation = paymentReceviedArr.every(item => item.installmentAmount);
        let sum = paymentReceviedArr.reduce((accumulator, curValue) => accumulator + parseInt(curValue.installmentAmount), 0)
        if (!validation) {
...

                {
                    isQuotationapproved ?
                        <div className="modal-body">
                            ...
                            <div className="row">
                                <div className="col-sm-6">
                                    <div className="form-group">
                                        <label className="col-form-label">Quotation Name : {selectedQuotation?.destinationName}</label>
                                    </div>
                                </div>
                                <div className="col-sm-6">
                                    <div className="form-group">
                                        <label className="col-form-label">Number Of Guest : {selectedQuotation?.numberOfGuest}</label>
                                    </div>
                                </div>
                                <div className="col-sm-6">
                                    <div className="form-group">
                                        <label className="col-form-label">Duration Of Tour: {selectedQuotation?.durationOfTour} </label>
                                    </div>
                                </div>
                                <div className="col-sm-6">
                                    <div className="form-group">
                                        <label className="col-form-label">Status:{selectedQuotation?.status}</label>
                                    </div>
                                </div>
                            </div>
                            <hr />
                            ...
                            {/* Add Client Modal */}
                            <div id="add_destination" className="modal custom-modal fade" role="dialog">
                                        ...
                                        <div className="modal-body">
                                            <form onSubmit={handleSubmit}>
                                                ...
                                                <div className="col-12">
                                                    <button data-bs-dismiss="modal" className='btn add-btn' onClick={handlePerfomaInvoiceSubmit}> Save </button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
